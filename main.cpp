...
//    }
    ...
    if(request.getUrl() == "/")
    {
        response->setStatusCode(HttpResponse::D_200_OK);
        response->setStatusMessage("OK");
        response->setContentType("text/html");
        response->addHeader("Server","sumer");
        std::string now = TimeUnit::now().ToFormatString();
    ...
    else if(request.getUrl() == "hello")
    {
        response->setStatusCode(HttpResponse::D_200_OK);
        response->setStatusMessage("OK");
        response->setContentType("image/plain");
        response->addHeader("Server","sumer");
