...
void http_parse::init(int efd,int fd){
    m_efd=efd;
    m_fd=fd;
...
bool http_parse::read_buf() {//循环读数据到读数据缓冲区，直到无数据可读，或对方关闭连接
    ...
    int read_bytes=0;
    while(true){
        read_bytes=recv(m_fd,m_read_buf+m_read_index,READ_BUF_SIZE-m_read_index,0);
        if(read_bytes==-1){
...
HTTP_CODE http_parse::process_read(){
...
//从状态机，用于解析一行的内容
HTTP_CODE http_parse::parse_request_line(char *line){
    m_url=strpbrk(line," \t");
    if(!m_url){
        return BAD_REQUEST;
    }
...
HTTP_CODE http_parse::parse_headers(char *line){
    if(line[0]=='\0'){
...
HTTP_CODE http_parse::parse_content(char *line){
...
http_parse::LINE_STATUS http_parse::parse_line(){
    char tmp;
...
HTTP_CODE http_parse::do_request(){
...
bool http_parse::write_buf(){//返回true则不关闭连接，返回false的话则关闭连接
    int tmp=0;
    int byte_have_send=0;
    if(m_write_index==0){//没有数据可发送，监听读事件
        modifyfd(m_efd,m_fd,EPOLLIN);
        init(m_efd,m_fd);
        return true;
    }
    while(true){
        tmp=writev(m_fd,m_iv,m_iv_cnt);
        if(tmp<0){
            //如果TCP没有缓冲空间，则等待下一轮EPOLLOUT事件。虽然在此期间服务器无法立即接收到同一客户的下一个请求，但这可保证连接的完整性。
            if(errno==EAGAIN){
                modifyfd(m_efd,m_fd,EPOLLOUT);
                return true;
            }
            unmap();
            return false;
...
bool http_parse::process_write(HTTP_CODE stat_code){
    switch (stat_code){
        case BAD_REQUEST:
...
bool http_parse::add_response(const char *format,...){//往缓冲区中写待发送的数据
...
bool http_parse::add_status_line(int status,const char* title){
    return add_response("%s %d %s\r\n","HTTP/1.1",status,title);
}

bool http_parse::add_headers(int content_len){
    add_content_len(content_len);
    add_linger();
    add_blank_line();
}

bool http_parse::add_content_len(int content_len){
    return add_response("Content-length:%d\r\n",content_len);
}

bool http_parse::add_linger(){
    return add_response("Connection:%s\r\n",(m_islinger?"keep-alive":"close"));
}

bool http_parse::add_blank_line(){
    return add_response("%s","\r\n");
}

bool http_parse::add_content(const char *content){
